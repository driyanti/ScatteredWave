      subroutine fcycle3(f,w,acor,dinv,dc,mmax,kmg,kmax,mx,my,md,
     &                    nz,nallx2,nally2,
...
c      
      integer     nz,nallx2,nally2,mx,my,kmg,idata 
      integer     nr1,nr2,nr3,nr4 
      complex*16  f(*),w((nz+2)*nallx2*nally2),r(*) 
...
c      do j = iys(ky)+1,iys(ky)+nyf
c         do i = ixs(kx)+1,ixs(kx)+nxf
c            do k = 1,nz
c	       write(50,*),k,i,j,f(k+(i+(j-1)*(nallx2)-1)*nz2+1)
c            enddo
c         enddo
c      enddo
...
c         call res27msgf(acor,w,f,r,nallx2,nally2,
c     &                  ixs(kx),iys(ky),nx(kx),ny(ky),nz, 
c     &                  neigz,neigx(2*kx), neigy(2*ky))
      else
         if (iswp .eq. 0) then
...
c         call res27msgc(acor,w,f,r,nallx2,nally2,
c     &                  ixs(kx),iys(ky),nx(kx),ny(ky),nz, 
c     &                  neigz,neigx(2*kx), neigy(2*ky))
      endif
c
      kmd = kmg/md
      if (md*kmd .ne. kmg) return
c
c     compute residual
      if (kmg .eq. mmax) then
         call res27msgf(!acor,
...
c     &              neigz,neigx(2*kx), neigy(2*ky))
c
c     transfer the residual r to the coarse grid
c
      call restr3a(r,f,wrst,nallx2,nally2,
     &             nx(kx-1),nx(kx),ixs(kx-1),ixs(kx),ix0(kx),
     &             ny(ky-1),ny(ky),iys(ky-1),iys(ky),iy0(ky),
...
c
      kmgc = kmg-1
c
      call vcycle3(f,w,acor,dinv,dc,mmax,kmgc,kmax,mx,my,md,
     &             nz,nallx2,nally2,nx,ny,idata,omg,bet1,bet2,
     &             ixs,iys,ix0,iy0,
     &             omega,iswp,0,
     &             nr1,nr2,nr3,nr4,
     ...
     &             ipe,ipz,npz)
c
c     prolong w_(kmg-1) and add it to w_kmg 
c
      call extup3a(w,r,witpd,nallx2,nally2, 
     &             nx(kx-1),nx(kx),ixs(kx-1),ixs(kx),ix0(kx),
     &	           ny(ky-1),ny(ky),iys(ky-1),iys(ky),iy0(ky),
