...
c
      ...
      integer     idata 
c      complex*16  a(0:nz+1,nallx2,nally2,27) 
...
c
     ...
     &                 nx,ny,nz)
                  work(k,i,j) = tempw 		  
c		  write(16,*),k,i,j,f(k,i,j),work(k,i,j),w(k,i,j)
               enddo
            enddo
         enddo
c
...
cs     &                    -dinv(k,i,j)*a(k,i,j,15)*work(k+1,i,j)
                  enddo
               enddo
            enddo
...
c
c               work(nz,i,j) = dinv(nz,i,j)*
c     &              (work(nz,i,j)-a(nz,i,j,13)*work(nz-1,i,j)
c     &              -a(nz,i,j,15)*work(nz+1,i,j))
c               work(nz+1,i,j) = work(nz,i,j)
            enddo
         enddo
c     
...
c
 500     continue
     ...
     &                    + omega*work(k,i,j)
	write(16,*),k,i,j,f(k,i,j),work(k,i,j),w(k,i,j)     
               enddo
            enddo
         enddo
